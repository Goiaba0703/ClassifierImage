...
class Class_Training(object):
    ...
    def div_training_test(self,images32, labels):
    ...
    def training(self,X_train, y_train,X_test, y_test, model, Monitor):
        #Training model
        #model.fit(X_train, y_train, batch_size = self.batch_size, epochs = self.epochs, verbose = self.verbose, validation_data = (X_test, y_test), callbacks = [Monitor])
